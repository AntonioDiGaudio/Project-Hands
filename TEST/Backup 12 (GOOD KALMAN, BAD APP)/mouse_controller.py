...
class MouseController:
    ...
    def move_cursor(self, ix, iy, frame_width, frame_height):
        """
        Muove il cursore del mouse in base alle coordinate dell'indice, usando il filtro di Kalman.
        """
        # Mappatura schermo con sensibilità
        ...
        z = np.array([zx, zy], dtype=np.float32)  # osservazione
        ...
        # Aggiornamento
        y = z - np.dot(self.H, self.xk)  # innovazione
        S = np.dot(self.H, np.dot(self.Pk, self.H.T)) + self.R  # covarianza dell'innovazione
        K = np.dot(np.dot(self.Pk, self.H.T), np.linalg.inv(S))  # guadagno di Kalman
        self.xk = self.xk + np.dot(K, y)  # stato aggiornato
        self.Pk = np.dot(self.I4 - np.dot(K, self.H), self.Pk)  # covarianza aggiornata

        # Posizione filtrata
        filtered_x = self.xk[0]
        filtered_y = self.xk[1]
