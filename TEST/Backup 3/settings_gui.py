...
"""
    ...
    try:
        with open(PROFILE_PATH, "r") as f:
            ...
            # Validazione valori
            config.alpha_smooth = max(0.1, min(1.0, settings.get("alpha_smooth", 0.3)))
            config.cursor_speed_multiplier = max(0.5, min(5.0, settings.get("cursor_speed_multiplier", 1.0)))

            # Aggiornamento GUI
            sliders['alpha_smooth'].set(config.alpha_smooth * 20)  # Mappatura su scala 1-20
