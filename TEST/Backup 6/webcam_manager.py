...
class WebcamManager:
    ...
    def get_camera_names(self):
        """
        Ottiene i nomi delle webcam disponibili su Linux utilizzando v4l2-ctl.
        
        Returns:
            list: Lista di stringhe con i nomi delle webcam trovate.
        """
    ...
    def try_camera(self, index, found, camera_names):
        """
        Tenta di aprire una webcam e verifica se è disponibile.
        
        Args:
            index (int): Indice della webcam da testare.
            found (list): Lista in cui aggiungere le webcam trovate.
            camera_names (list): Lista dei nomi delle webcam.
        """
        try:
            ...
            if cap.isOpened():
                ret, frame = cap.read()
                if ret:
                    camera_name = camera_names[index] if index < len(camera_names) else f"Webcam #{index}"
                    found.append((index, camera_name))
                cap.release()
            else:
                print(f"Webcam {index} non disponibile.")
        except cv2.error as e:
            print(f"Errore durante l'accesso alla videocamera {index}: {e}")
    
    def list_available_cameras(self, max_cams=2, stop_event=None):
        """
        Elenca le webcam disponibili nel sistema.
        
        Args:
            max_cams (int): Numero massimo di webcam da cercare.
            stop_event (threading.Event): Evento per interrompere la ricerca.
            
        Returns:
            list: Lista di tuple (indice, nome) delle webcam trovate.
        """
        found = []
        ...
        for i in range(max_cams):
            if stop_event and stop_event.is_set():
                break
            self.try_camera(i, found, camera_names or [])
        return found
...
# Funzione di utilità per selezionare una webcam (per compatibilità con il codice esistente)
